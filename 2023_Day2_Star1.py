...
def isPossible(pick):
    cubes, colour = pick.split(' ')
    return (colour == 'red' and int(cubes) <= 12) or (colour == 'green' and int(cubes) <= 13) or (colour == 'blue' and int(cubes) <= 14)
        
def getPossibleGameID(txt):
    gameDetails, game = txt.strip().split(': ')
    gameID = int(gameDetails.split(' ')[-1])
    for record in game.split('; '):
        for pick in record.split(', '):
            if not isPossible(pick):
                return 0
    return gameID

def getPossibleGamesSum(inputTextList):
    return sum(map(getPossibleGameID, inputTextList))

myInp = readInpText('day2inp11.txt')
print(getPossibleGamesSum(myInp))
